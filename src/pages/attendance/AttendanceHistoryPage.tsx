...
const AttendanceHistoryPage = () => {
  ...
  const { call: handleConfirm } = useAsync(
    async (list: AttendanceList) => {
      try {
        const presentIds = list.confirmedScouts.map(
          (scout: ScoutData) => scout.id
        );
        await scoutApi.confirmList(presentIds);
        toast({
          title: 'Lista confirmada com sucesso!',
          status: 'success',
          position: 'top',
        });
  ...
  const handleCardClick = useCallback(
    (list: AttendanceList) => {
      if (list.status === AttendanceStatus.InProgress) {
        const presentScouts = list.confirmedScouts;
        navigate('/listas/presenca/resumo', {
          state: { presentScouts, source: 'history' },
        });
      } else {
  ...
  const renderContent = () => {
      ...
      return (
        ...
        >
          <Text fontSize="lg" color="gray.500">
            Nenhuma lista de presença foi encontrada.
          </Text>
        </Box>
      );
    }
    ...
    return (
      <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} spacing={6}>
        {allLists.map((list: AttendanceList) => (
          <AttendanceCard
            key={list.id}
            listData={list}
            status={list.status}
