...
import type { ScoutData } from '../../services/scoutApi.types';
...
const AttendanceListPage = () => {
  ...
  const fetchScoutsCallback = useCallback(async () => {
    try {
      ...
      const sortedScouts = fetchedScouts.sort((a, b) =>
  ...
  const { call: loadScouts, loading: isLoading } = useAsync(
    fetchScoutsCallback,
    []
  );
  ...
  useDidMountAndUpdate(() => {
    ...
    const presentScoutsData = scouts.filter((scout) =>
      presentScoutIds.has(scout.id)
    );
  ...
  const handleCheckboxChange = (scoutId: number, isChecked: boolean) => {
  ...
  const handleSelectAll = (isChecked: boolean) => {
    if (isChecked) {
      const allIds = new Set(scouts.map((scout: ScoutData) => scout.id));
  ...
  const handleSubmit = () => {
    const presentScouts = scouts.filter((scout) =>
      presentScoutIds.has(scout.id)
    );
  ...
  const isAllSelected =
    scouts.length > 0 && presentScoutIds.size === scouts.length;
  const isIndeterminate =
    presentScoutIds.size > 0 && presentScoutIds.size < scouts.length;

  const renderButtons = () => {
    if (isEditMode) {
      return (
        <VStack spacing={4}>
          <Button w="full" size="lg" colorScheme="teal" onClick={handleSubmit}>
          ...
          {scouts.map((scout: ScoutData) => (
            ...
            >
              ...
              <Checkbox
                ...
                onChange={(e) =>
                  handleCheckboxChange(scout.id, e.target.checked)
                }
                colorScheme="teal"
                size="lg"
              />
