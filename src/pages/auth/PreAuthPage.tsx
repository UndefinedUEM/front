...
const PreAuthPage = () => {
  ...
  const { call: handleRequestCode, loading: isRequesting } =
    useAsync(async () => {
      if (!email) {
        toast({
      ...
      try {
        ...
        toast({
          ...
          description:
            'Se o e-mail estiver correto, um código de acesso foi enviado.',
          status: 'success',
          position: 'top',
        });
        setStep('verify');
  ...
  const { call: handleVerifyCode, loading: isVerifying } =
    useAsync(async () => {
      if (pin.length < 4) {
        toast({
          description: 'Por favor, digite os 4 dígitos.',
          status: 'warning',
          position: 'top',
        });
        return;
  ...
  const renderRequestStep = () => {
    return (
      ...
      >
        <Text color="gray.500">
          Por favor, insira o seu e-mail de acesso. Um código de verificação
          será enviado se o endereço estiver em nossa lista de permissões.
        </Text>
